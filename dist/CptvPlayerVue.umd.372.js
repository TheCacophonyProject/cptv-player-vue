...
class CptvDecoderInterface {
  async initWasm() {
    if (!initedWasm) {
      if (!!promises_ignored_) { // fs exists, so this is node
  ...
  async getBytesMetadata(fileBytes) {
    await this.initWithFileBytes(fileBytes, "", !!promises_ignored_);
...
/******/ 					var req = fetch(__webpack_require__.p + "" + {"711":{"1307":"388a37586be01d76d0db"}}[chunkId][wasmModuleId] + ".module.wasm");
/******/ 					var promise;
